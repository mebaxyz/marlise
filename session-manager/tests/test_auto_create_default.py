...
async def test_auto_create_default_pedalboard():
    ...
    try:
        assert main.session_manager is not None
        pb = getattr(main.session_manager, "current_pedalboard", None)
        assert pb is not None, "Expected a default pedalboard to be created"
        assert getattr(pb, "name", "") == "Default"
    finally:
        # Ensure clean shutdown
        await shutdown()
        # After shutdown the globals are cleared; nothing to assert here

        ...
        try {
            ...
            if (result) {
                ...
                try {
                    ...
                    if (json_req.isMember("action") && json_req["action"].asString() == "plugin") {
                    ...
                    } else if (json_req.isMember("action") && json_req["action"].asString() == "audio") {
                        json_resp = process_audio_command(json_req);
                    } else {
                        ...
                        // Process command
                        json_resp = process_command(cmd_req);
                    }

                    // Ensure canonical response envelope: normalize successful
                    // replies (plain objects or primitives) into {"success": true, ...}
                    if (json_resp.isObject()) {
                        if (!json_resp.isMember("success") && !json_resp.isMember("error")) {
                            Json::Value normalized(Json::objectValue);
                            normalized["success"] = true;
                            for (const auto& name : json_resp.getMemberNames()) {
                                normalized[name] = json_resp[name];
                            }
                            json_resp = normalized;
                        }
                    } else if (!json_resp.isNull()) {
                        // For non-object results (arrays/primitives), wrap as result
                        Json::Value normalized(Json::objectValue);
                        normalized["success"] = true;
                        normalized["result"] = json_resp;
                        json_resp = normalized;
                    }
...
Json::Value CommandService::process_audio_command(const Json::Value& request) {
    try {
        if (!request.isMember("method")) {
            throw std::runtime_error("Audio command missing 'method' field");
        }
        ...
        if (method == "init_jack") {
            response = InitJackResponse{audio_system_manager_->init()};
        } else if (method == "close_jack") {
            audio_system_manager_->close();
            response = CloseJackResponse{true};
        } else if (method == "get_jack_data") {
            bool with_transport = false;
            if (request.isMember("with_transport")) {
                with_transport = request["with_transport"].asBool();
            }
            auto data = audio_system_manager_->get_data(with_transport);
            response = GetJackDataResponse{std::move(data)};
        } else if (method == "get_jack_buffer_size") {
            unsigned buffer_size = audio_system_manager_->get_buffer_size();
            response = GetJackBufferSizeResponse{buffer_size};
        } else if (method == "set_jack_buffer_size") {
            if (!request.isMember("size")) {
                throw std::runtime_error("set_jack_buffer_size missing 'size' field");
            }
            unsigned size = request["size"].asUInt();
            unsigned buffer_size = audio_system_manager_->set_buffer_size(size);
            response = SetJackBufferSizeResponse{buffer_size};
        } else if (method == "get_jack_sample_rate") {
            float sample_rate = audio_system_manager_->get_sample_rate();
            response = GetJackSampleRateResponse{sample_rate};
        } else if (method == "get_jack_port_alias") {
            if (!request.isMember("port_name")) {
                throw std::runtime_error("get_jack_port_alias missing 'port_name' field");
            }
            std::string port_name = request["port_name"].asString();
            std::string alias = audio_system_manager_->get_port_alias(port_name);
            response = GetJackPortAliasResponse{alias};
        } else if (method == "get_jack_hardware_ports") {
            if (!request.isMember("is_audio") || !request.isMember("is_output")) {
                throw std::runtime_error("get_jack_hardware_ports missing 'is_audio' or 'is_output' field");
            }
            bool is_audio = request["is_audio"].asBool();
            bool is_output = request["is_output"].asBool();
            auto ports = audio_system_manager_->get_hardware_ports(is_audio, is_output);
            response = GetJackHardwarePortsResponse{ports};
        } else if (method == "has_midi_beat_clock_sender_port") {
            bool has_port = audio_system_manager_->has_midi_beat_clock_sender_port();
            response = HasMidiBeatClockSenderPortResponse{has_port};
        } else if (method == "has_serial_midi_input_port") {
            bool has_port = audio_system_manager_->has_serial_midi_input_port();
            response = HasSerialMidiInputPortResponse{has_port};
        } else if (method == "has_serial_midi_output_port") {
            bool has_port = audio_system_manager_->has_serial_midi_output_port();
            response = HasSerialMidiOutputPortResponse{has_port};
        } else if (method == "has_midi_merger_output_port") {
            bool has_port = audio_system_manager_->has_midi_merger_output_port();
            response = HasMidiMergerOutputPortResponse{has_port};
        } else if (method == "has_midi_broadcaster_input_port") {
            bool has_port = audio_system_manager_->has_midi_broadcaster_input_port();
            response = HasMidiBroadcasterInputPortResponse{has_port};
        } else if (method == "has_duox_split_spdif") {
            bool has_feature = audio_system_manager_->has_duox_split_spdif();
            response = HasDuoxSplitSpdifResponse{has_feature};
        } else if (method == "connect_jack_ports") {
            if (!request.isMember("port1") || !request.isMember("port2")) {
                throw std::runtime_error("connect_jack_ports missing 'port1' or 'port2' field");
            }
            std::string port1 = request["port1"].asString();
            std::string port2 = request["port2"].asString();
            bool success = audio_system_manager_->connect_ports(port1, port2);
            response = ConnectJackPortsResponse{success};
        } else if (method == "connect_jack_midi_output_ports") {
            if (!request.isMember("port")) {
                throw std::runtime_error("connect_jack_midi_output_ports missing 'port' field");
            }
            std::string port = request["port"].asString();
            bool success = audio_system_manager_->connect_midi_output_ports(port);
            response = ConnectJackMidiOutputPortsResponse{success};
        } else if (method == "disconnect_jack_ports") {
            if (!request.isMember("port1") || !request.isMember("port2")) {
                throw std::runtime_error("disconnect_jack_ports missing 'port1' or 'port2' field");
            }
            std::string port1 = request["port1"].asString();
            std::string port2 = request["port2"].asString();
            bool success = audio_system_manager_->disconnect_ports(port1, port2);
            response = DisconnectJackPortsResponse{success};
        } else if (method == "disconnect_all_jack_ports") {
            if (!request.isMember("port")) {
                throw std::runtime_error("disconnect_all_jack_ports missing 'port' field");
            }
            std::string port = request["port"].asString();
            bool success = audio_system_manager_->disconnect_all_ports(port);
            response = DisconnectAllJackPortsResponse{success};
        } else if (method == "reset_xruns") {
            audio_system_manager_->reset_xruns();
            response = ResetXrunsResponse{true};
        } else {
            throw std::runtime_error("Unknown audio command method: " + method);
        }
        ...
        Json::Value error_resp;
        error_resp["error"] = std::string("Audio command failed: ") + e.what();
        return error_resp;
    }
}
